...
const recentRecipientMessages = new Map(); // jid -> array of timestamps
const recentDistinctRecipients = new Map(); // minute-windowed set counter per minute
const recentMessageHistory = new Map(); // jid -> array of {hash, ts}

function addRecentRecipientMessage(jid) {
    const now = Date.now();
    const log = recentRecipientMessages.get(jid) || [];
    const nextLog = log.filter(t => now - t < 60000);
    nextLog.push(now);
    recentRecipientMessages.set(jid, nextLog);
    return nextLog.length;
}

function addDistinctRecipient(jid) {
    const now = Date.now();
    const minuteKey = Math.floor(now / 60000);
    const bucket = recentDistinctRecipients.get(minuteKey) || new Set();
    bucket.add(jid);
    recentDistinctRecipients.set(minuteKey, bucket);
    // cleanup old keys
    for (const key of Array.from(recentDistinctRecipients.keys())) {
        if (key < minuteKey - 1) recentDistinctRecipients.delete(key);
    }
    return bucket.size;
}

function recordMessageHistory(jid, hash) {
    const now = Date.now();
    const history = recentMessageHistory.get(jid) || [];
    const windowMs = config.anti.repeatedMessagesWindowMs;
    const nextHistory = history.filter(item => now - item.ts < windowMs);
    nextHistory.push({ hash, ts: now });
    recentMessageHistory.set(jid, nextHistory);
    return nextHistory.filter(item => item.hash === hash).length;
}

function getRepeatedMessageCount(jid, hash) {
    const now = Date.now();
    const history = recentMessageHistory.get(jid) || [];
    const windowMs = config.anti.repeatedMessagesWindowMs;
    return history.filter(item => item.hash === hash && now - item.ts < windowMs).length;
}

function hashMessageContent(content) {
    const payload = content?.text || JSON.stringify(content || '');
    let hash = 0;
...
function wrapSockForAntiban(sock) {
    ...
    sock.sendMessage = async (jid, content, options) => {
        ...
        // Report protection heuristics: ekstra delay pada pola pengiriman yang berisiko.
        if (config.anti.reportProtection) {
            const recipientCount = addRecentRecipientMessage(jid);
            const distinctRecipients = addDistinctRecipient(jid);
            const hash = hashMessageContent(content);
            const repeatedCount = recordMessageHistory(jid, hash);

            if (repeatedCount > config.anti.repeatedMessagesThreshold) {
                await humanDelay(config.anti.highRiskRecipientDelayMs, config.anti.highRiskRecipientDelayMs + 1200);
            }
            if (recipientCount > config.anti.maxRecipientMessagesPerMinute) {
                await humanDelay(config.anti.extraDelayMs, config.anti.extraDelayMs + 1800);
            }
            if (distinctRecipients > config.anti.maxDistinctRecipientsPerMinute) {
                await humanDelay(config.anti.extraDelayMs + 1500, config.anti.highRiskRecipientDelayMs + 2400);
            }
        }
